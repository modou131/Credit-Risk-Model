...
from predtion_helper import predict

st.title('MJ Finance: Credit Risk Model')
...
with row3[2]:
    num_open_accounts = st.number_input('Number of open accounts', min_value=1, max_value=4, step=1, value=2)

with row4[0]:
    residence_type = st.selectbox('Residence Type', options=['Owned', 'Rented', 'Mortgage'])
with row4[1]:
    loan_purpose = st.selectbox('Loan Purpose', options=['Education', 'Home', 'Auto', 'Personal'])
with row4[2]:
